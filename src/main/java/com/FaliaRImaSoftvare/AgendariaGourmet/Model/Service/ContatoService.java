...
import com.FaliaRImaSoftvare.AgendariaGourmet.Model.Exception.AcessoIlegalException;
...
public class ContatoService {
	...
	public Long criar(ContatoDTO con, Long userid) {
		...
		if (con.DDD() == null || con.DDD().isBlank())
			throw new CamposInvalidosException("DDD branco ou nulo");
		if (con.Email() == null || con.Email().isBlank())
			throw new CamposInvalidosException("Email branco ou nulo");
		if (con.nomeContato() == null || con.nomeContato().isBlank())
			throw new CamposInvalidosException("Nome do contato branco ou nulo");
		if (con.telefone() == null || con.telefone().isBlank())
			throw new CamposInvalidosException("numero de telefone branco ou nulo");
		if (userid == null)
			throw new CamposInvalidosException("Usuario invalido");
	...
	public void alterar(long id, String novoNome) {
		...
		if (id == 0)
			throw new CamposInvalidosException("id invalido");
		if (novoNome == null || novoNome.isBlank())
			throw new CamposInvalidosException("novo nome branco ou nulo");
	...
	public Contato consultar(Long userId, Long id) throws Exception {
		...
		if (contato.getUsuarioId() != userId)
			throw new AcessoIlegalException("O contato não pertence ao usuário");
	...
	public List<Contato> filtrar(Long userId, String filtro) {
		...
		if (filtro != null && filtro.isBlank() == false) {

			List<Contato> lista = fachada.filtrar(userId, filtro);
